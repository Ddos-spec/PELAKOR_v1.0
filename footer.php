<br><br>
<footer class="page-footer blue darken-2">
    <div class="container">
        <div class="row">
            <div class="col l6 s12">
            <h5 class="white-text">Company Bio</h5>
            <p class="grey-text text-lighten-4">
                pelakorKu adalah sebuah projek yang ditujukan sebagai tugas akhir untuk UKOM.<br/>
                Terima Kasih buat diri sendiri udah mau bantuin bikin web ini sama kepada adam yang telah membantu membuatkan desainnya.
            </p>


            </div>
            <div class="col l3 s12">
            <h5 class="white-text">Laundryku</h5>
            <ul>
                <li><a class="white-text" href="index.php">Home</a></li>
                <li><a class="white-text" href="term.php">Term and Condition</a></li>
                <li><a class="white-text" href="registrasi.php">Registrasi</a></li>
                <li><a class="white-text" href="login.php">Login</a></li>
            </ul>
            </div>
            <div class="col l3 s12">
            <h5 class="white-text">Developer</h5>
            <ul>
                <li><a class="white-text" href="team.php">Fadjar Aryo Seto</a></li>
                <li><a class="white-text" href="team.php">Adam</a></li>
                
            </ul>
            </div>
        </div>
    </div>
    <div class="footer-copyright">
        <div class="container">
            Made by <a class="orange-text text-lighten-3" href="http://materializecss.com">Materialize</a>
        </div>
    </div>
</footer>

<!--  Scripts-->
<script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
<!-- Compiled and minified JavaScript -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/js/materialize.min.js"></script>
<!-- end script -->